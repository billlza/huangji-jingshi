        ...
        {sun && (
          <div className="flex justify-between items-center text-yellow-200/80">
            <span className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-yellow-500"></span> {rename('Sun')}
            </span>
            <span>
              Alt {sun.alt_deg.toFixed(1)}° / Az {sun.az_deg.toFixed(1)}°
            </span>
          </div>
        )}
        {moon && (
          <div className="flex justify-between items-center text-gray-300">
            <span className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-gray-400"></span> {rename('Moon')}
            </span>
            <span>
              Alt {moon.alt_deg.toFixed(1)}° / Az {moon.az_deg.toFixed(1)}°
            </span>
          </div>
        )}
        {planets.map((p) => (
          <div
            key={p.name}
            className="flex justify-between items-center text-cyan-300/70 col-span-2 sm:col-span-1"
          >
            <span className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-cyan-500"></span> {rename(p.name)}
            </span>
