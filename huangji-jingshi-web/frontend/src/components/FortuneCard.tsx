...
const FortuneCard: React.FC<FortuneCardProps> = ({ data, currentYear, onJumpToYear }) => {
  ...
  const [mirrorItems, setMirrorItems] = useState<
    Array<{ year: number; title: string; dynasty?: string; person?: string }>
  >([]);
  const API_BASE =
    ...
    const run = async () => {
      try {
        ...
        const resp = await fetch(
          `${API_BASE}/api/history/related?year=${displayYear}&mode=yun&limit=3`,
        );
        if (!resp.ok) return;
        const list: Array<{ year: number; title: string; dynasty?: string; person?: string }> =
          await resp.json();
        setMirrorItems(list);
      } catch {
        setMirrorItems([]);
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-3xl overflow-hidden flex flex-col">
        ...
        <div className="p-8 border-b border-white/10 flex items-center justify-between bg-white/5">
          <div>
            <div className="text-xs text-gold/60 uppercase tracking-[0.3em] mb-3">
              值年卦象 (Annual Hexagram)
            </div>
            <h2 className="text-4xl font-serif text-white tracking-widest drop-shadow-lg mb-2">
              {data.hexagram_major}
            </h2>
      ...
      {detailsOpen && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-[420px] bg-gray-950 border-l border-white/10 p-6 overflow-y-auto">
            ...
            <pre className="text-[10px] bg-black/40 p-3 rounded border border-white/5 overflow-auto">
              {JSON.stringify(data.mapping_record || {}, null, 2)}
            </pre>
            {data.period_info && (
              <div className="mt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-[12px]">
                  <div className="bg-black/30 p-2 rounded border border-white/5">
                    ...
                    <div>
                      {data.period_info.hui.name} · {data.period_info.hui.start_year} –{' '}
                      {data.period_info.hui.end_year}
                    </div>
                  </div>
                  <div className="bg-black/30 p-2 rounded border border-white/5">
                    <div className="text-gray-500">运</div>
                    <div>
                      {data.period_info.yun.name} · {data.period_info.yun.start_year} –{' '}
                      {data.period_info.yun.end_year}
                    </div>
                  </div>
                  <div className="bg-black/30 p-2 rounded border border-white/5">
                    <div className="text-gray-500">世</div>
                    <div>
                      {data.period_info.shi.name} · {data.period_info.shi.start_year} –{' '}
                      {data.period_info.shi.end_year}
                    </div>
                  </div>
                  <div className="bg-black/30 p-2 rounded border border-white/5">
                    <div className="text-gray-500">旬</div>
                    <div>
                      {data.period_info.xun.name} · {data.period_info.xun.start_year} –{' '}
                      {data.period_info.xun.end_year}
                    </div>
                  </div>
                </div>
              </div>
...
const Chip: React.FC<{ label: string; value: string; color: string }> = ({
  ...
  <div className="flex items-center bg-white/5 rounded-full px-3 py-1 border border-white/5 hover:bg-white/10 transition-colors cursor-help group relative whitespace-nowrap">
    <span className="text-[10px] text-gray-500 mr-2 uppercase tracking-wider shrink-0">
      {label}
    </span>
...
function getTooltipText(label: string): string {
  switch (label) {
    case '元':
      return 'Epoch: 129600 years';
    case '会':
      return 'Era: 10800 years';
    case '运':
      return 'Cycle: 360 years';
    case '世':
      return 'Generation: 30 years';
    case '旬':
