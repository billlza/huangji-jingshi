...
export function StarMap({
  ...
  useEffect(() => {
    ...
    const resolveDataRoot = async (): Promise<string> => {
        ...
        try {
          const checks = await Promise.all(
            required.map(async (path) => {
              const response = await fetch(`${root}${path}`, { method: 'GET' });
              if (!response.ok) return false;
              const text = await response.text();
              if (!text || text.startsWith('404')) return false;
              let payload: unknown;
              try {
      ...
      const availableLocal = localProbeResults.find((entry) => entry.ok);
      if (availableLocal) {
        if (availableLocal.root === '/data/') {
          console.log('[StarMap] Using local static: /data/');
        } else if (availableLocal.root === '/api/celestial/data/') {
          console.log('[StarMap] Using local API: /api/celestial/data/');
        } else {
          console.log(`[StarMap] Using API_BASE: ${availableLocal.root}`);
        }
    ...
    const configBase = {
      ...
      stars: {
        ...
        namestyle: {
          fill: '#f6e58d',
          font: "bold 10px 'Lucida Sans Unicode', Trebuchet, Helvetica, Arial, sans-serif",
          align: 'left',
          baseline: 'top',
        },
        designation: false,
    ...
    const boot = async () => {
      ...
      window.alert = (msg?: unknown) => {
        ...
        if (s.includes('Data file could not be loaded')) {
          ...
          try {
            originalAlert(msg as unknown as string);
          } catch {
            void 0;
          }
        }
      ...
      const drawCn = async (base: string) => {
        try {
          ...
          if (
            lines &&
            window.Celestial &&
            typeof window.Celestial.add === 'function' &&
            showConst
          ) {
      ...
      const drawHjStars = async (base: string) => {
        try {
          const hj = await fetch(`${base}cultures/huangji-stars.json`).then((r) =>
            r.ok ? r.json() : null,
          );
          ...
          try {
            ...
            if (stars && Array.isArray(stars.features)) {
              for (const f of stars.features as Array<{
                id?: number;
          ...
          window.Celestial.add(resolved, {
            ...
            labels: true,
            label: (d: { properties?: { name_hj?: string; name_cn?: string } }) =>
              d.properties?.name_hj || d.properties?.name_cn || '',
            labelStyle: {
              fill: '#ffeaa7',
          ...
          setTimeout(() => {
            ...
            if (hjStarGroups.length > 0) {
              hjStarGroups.forEach((g) => {
                ...
                const data =
                  (g as unknown as DataBound).__data__ || (circle as unknown as DataBound).__data__;
            ...
            if (labelGroup!.children.length === 0) {
              ...
              allCircles.forEach((circle) => {
                ...
                // Get name from bound data
                const data =
                  (circle as unknown as DataBound).__data__ ||
                  (circle.parentElement as unknown as DataBound)?.__data__;
                const name = data?.properties?.name_hj || data?.properties?.name_cn || '';
                if (!name) return;
      ...
      const drawXiuPrincipal = async () => {
        ...
        const mansions: Array<{ key: string; name: string; idx: number }> = [
          { key: 'jiao', name: '角', idx: 1 },
          { key: 'kang', name: '亢', idx: 2 },
          { key: 'di', name: '氐', idx: 3 },
          { key: 'fang', name: '房', idx: 4 },
          { key: 'xin', name: '心', idx: 5 },
          { key: 'wei', name: '尾', idx: 6 },
          { key: 'ji', name: '箕', idx: 7 },
          { key: 'dou', name: '斗', idx: 8 },
          { key: 'niu', name: '牛', idx: 9 },
          { key: 'nv', name: '女', idx: 10 },
          { key: 'xu', name: '虚', idx: 11 },
          { key: 'wei2', name: '危', idx: 12 },
          { key: 'shi', name: '室', idx: 13 },
          { key: 'bi', name: '壁', idx: 14 },
          { key: 'kui', name: '奎', idx: 15 },
          { key: 'lou', name: '娄', idx: 16 },
          { key: 'wei3', name: '胃', idx: 17 },
          { key: 'mao', name: '昴', idx: 18 },
          { key: 'bi2', name: '毕', idx: 19 },
          { key: 'zi', name: '觜', idx: 20 },
          { key: 'shen', name: '参', idx: 21 },
          { key: 'jing', name: '井', idx: 22 },
          { key: 'gui', name: '鬼', idx: 23 },
          { key: 'liu', name: '柳', idx: 24 },
          { key: 'xing', name: '星', idx: 25 },
          { key: 'zhang', name: '张', idx: 26 },
          { key: 'yi', name: '翼', idx: 27 },
          { key: 'zhen', name: '轸', idx: 28 },
        ];
        const L0 = 0;
        const span = 360 / 28; // 12.857142...
        ...
        const fc = { type: 'FeatureCollection', features: feats } as unknown as Record<
          string,
          unknown
        >;
        ...
        window.Celestial.add(fc, {
          ...
          labelStyle: {
            fill: '#f6e58d',
            font: "bold 12px '-apple-system','SF Pro Text','Segoe UI', Roboto, sans-serif",
            align: 'left',
            baseline: 'middle',
          },
        } as unknown as Record<string, unknown>);
      ...
      const drawHjStarsDirect = async (base: string) => {
        try {
          ...
          const starsData = await fetch(`${base}stars.6.json`).then((r) =>
            r.ok ? r.json() : null,
          );
          ...
          console.warn('[StarMap] drawHjStarsDirect error:', e);
        }
      };
      ...
      try {
        if (window.Celestial && typeof window.Celestial.skyview === 'function') {
          window.Celestial.skyview({
            date: safeDate,
            location: [actualLat, actualLon],
            timezone: tzMinutes,
          });
        } else {
          if (window.Celestial && typeof window.Celestial.date === 'function')
            window.Celestial.date(safeDate);
          if (window.Celestial && typeof window.Celestial.location === 'function')
            window.Celestial.location([lat, lon]);
        }
      } catch {
        /* noop */
      }
      ...
      const ensureRendered = async () => {
        ...
        const vis = (el: Element | null) =>
          !!el && el.getBoundingClientRect().width > 20 && el.getBoundingClientRect().height > 20;
        const hasVisible = vis(canvas) || vis(svg);
        if (hasVisible) return;
        const fallback = {
          ...configBase,
          culture,
          datapath: rootRef.current ?? root,
          container: containerId,
          width: Math.max(720, w),
          projection: 'aitoff',
          constellations: {
            ...configBase.constellations,
            show: isIau ? showConst : false,
    ...
    if (container) {
      ...
      const applyOnlyHjVisibility = (only: boolean) => {
        ...
        const circles = Array.from(
          (svg.querySelectorAll('circle') || []) as NodeListOf<SVGCircleElement>,
        );
        circles.forEach((el) => {
        ...
        const texts = Array.from(
          (svg.querySelectorAll('text') || []) as NodeListOf<SVGTextElement>,
        );
        texts.forEach((el) => {
          const keep =
            !!el.closest('g[data-hj="1"]') ||
            el.classList.contains('hj-star') ||
            el.classList.contains('xiu-point');
          el.style.opacity = only && !keep ? '0' : '';
        });
      };
      setTimeout(() => {
        ...
        const renamePlanets = () => {
          ...
          texts.forEach((t) => {
            const zh = map[t.textContent || ''];
            if (zh) t.textContent = zh;
          });
        };
      ...
      setTimeout(() => {
        ...
        const points = Array.from(
          (svg?.querySelectorAll('.hj-star') || []) as NodeListOf<SVGElement>,
        );
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', background: '#000' }}>
      ...
      >
        ...
        >
          <input
            ...
            onChange={(e) => {
              ...
              if (API_BASE && useRemoteSettings) {
                try {
                  void fetch(`${API_BASE}/api/settings/sky`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      show_const: v,
                      show_xiu: showXiu,
                      zh_planet_names: renameZh,
                      culture,
                    }),
                    keepalive: true,
                  });
        ...
        <label
          style={{ color: '#f6e58d', fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}
        >
          文化
          <select
            value={culture}
            onChange={(e) => {
              const v = e.target.value as 'cn' | 'hj' | 'iau';
              setCulture(v);
              const API_BASE =
                (import.meta as unknown as { env?: Record<string, string> }).env
                  ?.VITE_BACKEND_URL || '';
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const whites = Array.from(
                (svg.querySelectorAll('circle') || []) as NodeListOf<SVGCircleElement>,
              ).filter((c) => getComputedStyle(c).fill === 'rgb(255, 255, 255)');
              const cnPts = Array.from(
                (svg.querySelectorAll('circle') || []) as NodeListOf<SVGCircleElement>,
              ).filter((c) => getComputedStyle(c).fill === 'rgb(246, 229, 141)');
              const labels = Array.from(
                (svg.querySelectorAll('text') || []) as NodeListOf<SVGTextElement>,
              ).filter((t) => getComputedStyle(t).fill === 'rgb(246, 229, 141)');
              const all = [...whites, ...cnPts, ...labels];
              all.forEach((el) => {
          ...
          onChange={(e) => {
            ...
            points.forEach((el) => {
              const feat = (
                el as {
                  __data__?: {
                    properties?: { name_hj?: string; name_cn?: string; hip?: number | string };
                  };
                }
              ).__data__;
              const name = feat?.properties?.name_hj || feat?.properties?.name_cn || '';
        ...
        <button
          ...
          style={{
            background: playing ? '#f6e58d22' : 'transparent',
            color: '#f6e58d',
            border: '1px solid #f6e58d',
            borderRadius: 4,
            fontSize: 11,
        ...
        <button
          ...
          disabled={gpsLoading}
          style={{
            background: gpsLocation ? '#4ade8022' : 'transparent',
            color: gpsLocation ? '#4ade80' : '#f6e58d',
            border: `1px solid ${gpsLocation ? '#4ade80' : '#f6e58d'}`,
            borderRadius: 4,
            fontSize: 11,
            height: 26,
            minWidth: 35,
          ...
          title={
            gpsLocation
              ? `GPS: ${gpsLocation.lat.toFixed(2)}°, ${gpsLocation.lon.toFixed(2)}°`
              : '获取GPS定位'
          }
        >
          {gpsLoading ? '...' : '📍'}
