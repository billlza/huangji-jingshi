...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const waitForSession = async (
      ...
      return waitForSession(getSession, attemptsLeft - 1);
    };
    ...
    try {
      ...
      if (data?.session || data?.user) {
        console.log(
          '[Login] Login successful, session:',
          data.session ? 'exists' : 'missing',
          'user:',
          data.user ? 'exists' : 'missing',
        );
        ...
        if (data.user) {
          ...
          const hasSession = await waitForSession(() => supabase.auth.getSession(), 10);
          if (hasSession) {
          ...
          console.log(
            '[Login] No session after wait, but user exists. Auth listener should handle navigation.',
          );
      ...
      if (msg.includes('Invalid login credentials') || msg.includes('invalid_credentials')) {
        ...
        msg =
          '请先确认您的邮箱（检查收件箱和垃圾邮件文件夹）。如果已确认，请尝试刷新页面后重新登录。';
      ...
      <div className="relative z-10 w-full max-w-md glass-panel border border-white/10 p-10 rounded-3xl shadow-2xl">
        <div className="text-center mb-10">
          ...
          <div className="w-16 h-16 mx-auto mb-6 relative flex items-center justify-center">
            ...
            <svg
              className="w-10 h-10 text-gold relative z-10"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {error && (
          <div
            className={`mb-6 p-4 rounded-lg text-xs text-center backdrop-blur-sm ${
              isSuccess
        ...
        <form className="space-y-6" onSubmit={handleLogin}>
          <div className="space-y-2">
            <label className="block text-gold/70 text-[10px] uppercase tracking-widest ml-1">
              邮箱 Email
            </label>
          ...
          <div className="pt-4 space-y-4">
            ...
            <div className="relative py-3">
              ...
              <div className="relative flex justify-center">
                <span className="px-4 bg-[#050508] text-[10px] text-gray-500 uppercase tracking-widest">
                  或者
                </span>
              </div>
            </div>
